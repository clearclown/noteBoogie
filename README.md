...
**紙のビジネス書（スキャンPDF・縦書き）を、章ごとの「メンター独話オーディオブック」と質問できるナレッジベースに変換する個人プロジェクト。**
...
- **DB**: SurrealDB。追加マイグレーションは 24（audiobook/章リンク+プロファイル）・25（book_figure+図ナレーション指示）・26（章の生成エラー可視化）
...
```bash
uv run pytest tests/                 # Python（700+）
cd gateway && cargo test             # Rust gateway（モックsidecar統合テスト含む）
cd frontend && npm test              # vitest（169）
