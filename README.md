...
8. **メンターAI（師匠）**: 蔵書全体 + 長期記憶（過去の相談）を持つコンサルの師匠と壁打ち。本/章単位の**学習傾斜**（どの本を重く参照するか 0〜2 + 自動傾斜）と回答の**TTS音声化**に対応。REST API（`/api/mentor/*`）と MCP（`consult_mentor`）で利用可、専用UI（/mentor）は開発中
