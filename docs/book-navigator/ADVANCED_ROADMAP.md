...
## 4. 品質ゲートの生成ループ組み込み + Self-RAG（次の一手・2026-07-23 壁打ち）
...
1. **採点のゲート化（生成ループ内）**: 現在の eval_transcript は事後測定。これを
   sidecar の生成直後に挟み、composite が閾値未満なら「指摘を briefing_suffix に
   付けて1回だけ再生成 → それでも未達なら generation_error で棄却」する。
   スライドレビューの品質ゲート（§C3、閾値3.0で未達バッジ+最優先の直し）と同じ
   思想を音声生成にも通す。
   - 論点: 閾値（初期は 0.6?）、再生成の上限回数（コスト暴走防止で1回）、
     ゲート指標に judge を含めるか（トークン費 vs 精度）
2. **Self-RAG 的な「答えられない判定」（chat/ask 側）**: 検索ヒットの
   最大 similarity とヒット数が下限未満なら、生成に入る前に
   「蔵書に十分な根拠がありません」と返す分岐をグラフに足す。
   メンターは既に「蔵書に無いことは一般論と明示」をプロンプトで課しているが、
   これを**プロンプト頼みでなく検索スコアの決定的分岐**にする。
   - 論点: 下限値の較正方法（既存の819チャンクで ROC を取る）、
     「一般論として答える」フォールバックを許すか
3. **判定の永続化**: ゲート判定・棄却理由を episode / chat ログに残し、
   閾値較正のデータにする（RL段階3の報酬モデル蒸留と同じデータが使える）
